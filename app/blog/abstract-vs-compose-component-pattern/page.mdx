...
Since we aim to only have a small wrapper around core JSX components, this means it's less likely we'll need to alter our core abstraction. Changes we need to make can be passed in as props as we compose an individual input field. That doesn't mean there will never be a case where we need to update a primitive component, but it was be far, far less frequent that with the abstract pattern.
...
With our previous abstraction, we had nightmares about 100 props on the component but what if we made our props modular? There's a very clear separation of concerns despite us abstracting our component:
